...
public interface MeasureBatch {
   ...
   * Records list of measurements for each Measures associated with {@code MeasureBatch}.
   *
   * @param numbers the list of number (double or long) that is recorded for each Measure ({code
   *     MeasureDouble} or {@code MeasureLong} associated with this {@code MeasureBatch}.
   *     Measurements must be in same order as the order in which measures were added to the batch.
   * @param distContext the distContext associated with the measurements.
   * @param attachmentValue the attachment value for exemplar.
